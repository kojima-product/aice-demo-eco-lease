...
## 2025-11-23 (更新17): 🎯 見積精度向上Phase2 - チェックリスト・数量推定・UI改善

### 実施した改善

#### 1. 工事区分別チェックリストの実装 ✅

**新規ファイル**: `pipelines/estimation_rules.py`

各工事区分の必須項目をチェックリスト化し、項目の網羅性を検証・補完する機能を実装。
...
**電気設備工事のカテゴリ例**:
- 受変電設備: キュービクル、PAS、変圧器、接地工事等
- 幹線設備: 幹線ケーブル、ケーブルラック、配管・配線等
- 照明設備: 照明器具、非常照明、誘導灯、スイッチ等
- コンセント設備: コンセント、OAコンセント等
- 弱電設備: 電話、LAN、インターホン、放送設備等
- 防災設備: 自動火災報知設備、非常放送設備等

---

#### 2. 数量推定ルールの実装 ✅

**機能**: 数量が未設定の項目に対し、建物情報（床面積・部屋数・階数）から数量を自動推定。

**推定ルール例**:
| 項目 | 推定基準 | 計算式 |
|------|---------|--------|
| 照明器具 | 床面積 | 8台/100㎡ |
| コンセント | 床面積/部屋数 | 15個/100㎡ または 4個/室 |
| スイッチ | 部屋数 | 2個/室 |
| 分電盤 | 階数 | 2面/階 |
| ケーブル | 床面積 | 50m/100㎡ |

**推定数量の信頼度**: 0.6（手動入力より低く設定）

---

#### 3. ㎡単価妥当性検証の実装 ✅

**機能**: 工事区分ごとの合計金額を床面積で割り、建物タイプ別の期待範囲と比較。

**建物タイプ別㎡単価目安（学校）**:
| 工事区分 | 最小 | 最大 | 標準 |
|---------|------|------|------|
| 電気設備工事 | ¥15,000/㎡ | ¥40,000/㎡ | ¥25,000/㎡ |
| 機械設備工事 | ¥20,000/㎡ | ¥50,000/㎡ | ¥35,000/㎡ |
| ガス設備工事 | ¥1,000/㎡ | ¥8,000/㎡ | ¥3,000/㎡ |

---

#### 4. 見積生成フローへの統合 ✅

**estimate_generator_ai.py の更新**:
...
4. KBから単価マッチング
5. 妥当性検証（㎡単価チェック） ← NEW
6. FMTDocument生成（検証結果をmetadataに含む）
```
...
- **チェックリスト網羅性表示**: 工事区分ごとのカバー率、不足項目一覧
- **㎡単価検証表示**: 妥当性判定結果（✓妥当/⚠高すぎ/⚠低すぎ）
- **項目一覧のNo.列追加**: 階層番号（1, 1.1, 1.1.1形式）を表示
- **階層インデント**: 項目名に階層レベルに応じたインデントを適用

---

#### 6. その他の修正 ✅

**ナンバリング機能** (`_assign_item_numbers`):
- 階層構造に基づいて 1, 1.1, 1.1.1 形式の番号を自動付与
- 工事区分ごとに独立してナンバリング

**重複計算の修正**:
- 工事区分別内訳でLevel 0のみを合計（子項目の重複計算を防止）
...
| ファイル | 変更内容 |
|---------|---------|
| ✅ `pipelines/estimation_rules.py` | **新規作成** - チェックリスト、数量推定、㎡単価検証 |
| ✅ `pipelines/estimate_generator_ai.py` | チェックリスト統合、ナンバリング機能追加 |
| ✅ `pipelines/schemas.py` | `estimation_basis`フィールド追加 |
| ✅ `pipelines/pdf_generator.py` | リース期間の改行対応 |
| ✅ `pages/1.py` | チェック結果表示、No.列追加、重複計算修正 |
...
| テスト項目 | 結果 |
|-----------|------|
| チェックリスト読込 | ✓ 電気36項目/機械30項目/ガス21項目 |
| 数量推定 | ✓ 面積・部屋数ベースで動作 |
| ㎡単価検証 | ✓ 妥当性判定が動作 |
| PDF生成 | ✓ 正常生成 |
| 全モジュールインポート | ✓ エラーなし |
...
| 改善項目 | 期待効果 |
|---------|---------|
| チェックリスト | 項目漏れ防止（網羅性向上） |
| 数量推定ルール | 数量精度向上（面積/部屋数ベース） |
| ㎡単価検証 | 異常値検出（見積妥当性チェック） |
| ナンバリング | 見積書の可読性向上 |
...
1. ⬜ **図面Vision解析強化**: 配管ルート・長さの自動読み取り
2. ⬜ **類似案件比較機能**: 過去の類似プロジェクトとの比較

---

## 2025-11-22 (更新16): 🎯 見積精度向上機能の実装

### 実施した改善

#### Phase 1: 諸元表のVision抽出強化 ✅

**新機能**: `extract_specification_table_with_vision()`

PDFの諸元表ページ（39-40ページ）を画像として抽出し、Claude Vision APIで構造化データに変換。
...
**改善効果**: テキストベースの抽出では取得できなかった表形式データを正確に抽出可能に。

---

#### Phase 2: KBマッチングロジック改善 ✅

**2.1 類義語辞書 `SYNONYM_DICT`**

項目名の類義語を定義し、マッチング精度を向上:
- 「高圧気中負荷開閉器（PAS）」→「気中開閉器」等6件にマッチ
- 「キュービクル」→「受変電設備」「高圧受電設備」等にマッチ
- 「架橋ポリエチレンケーブル」→「CV」「CVT」等にマッチ

**2.2 discipline互換性チェック緩和**
...
**テスト結果**:
- KB=「設備工事」→ 電気設備工事、ガス設備工事にマッチ ✓
- KB=「電気設備工事」→ 同じdisciplineにマッチ ✓

**2.3 単価妥当性チェック `HIGH_VALUE_ITEMS`**
...
- ✅ `pipelines/estimate_generator_ai.py`:
  - 類義語辞書 `SYNONYM_DICT` 追加
  - 高額機器リスト `HIGH_VALUE_ITEMS` 追加
  - `_is_discipline_compatible()` メソッド追加
  - `_find_synonyms()` メソッド追加
  - `_validate_price()` メソッド追加
  - `extract_specification_table_with_vision()` メソッド追加
  - `enrich_with_prices()` 更新（類義語/discipline/価格検証）
  - `generate_estimate()` 更新（Vision抽出統合）

- ✅ `IMPLEMENTATION_PLAN_ACCURACY.md`: 実装計画・結果ドキュメント
- ✅ `test_accuracy_improvement.py`: テストスクリプト
...
| 指標 | 改善前 | 改善後 |
|------|--------|--------|
| 諸元表抽出 | テキストベース（不正確） | Vision API（79部屋タイプ抽出） |
| discipline互換性 | 完全一致のみ | 緩和版（設備工事→全設備） |
| 類義語マッチング | なし | 辞書ベース（6件/項目） |
