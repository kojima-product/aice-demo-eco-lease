...
SYNONYM_DICT = {
    ...
    "架橋ポリエチレンケーブル": ["CV", "CVT", "高圧ケーブル", "CVケーブル", "CVTケーブル", "CVQ", "CVD", "架橋ポリエチレン絶縁ケーブル"],
    "架橋ポリエチレン絶縁ケーブル": ["架橋ポリエチレンケーブル", "CV", "CVT", "高圧ケーブル", "CVケーブル"],
    ...
    "消火器": ["ABC消火器", "粉末消火器"],
    # ガス設備
    "白ガス管": ["鋼管", "ガス管", "SGP", "配管用炭素鋼鋼管", "白管", "白ガス管（ネジ接合）", "白ガス管(ネジ接合)"],
    "白ガス管（ネジ接合）": ["白ガス管", "鋼管", "ガス管", "SGP", "配管用炭素鋼鋼管", "白管"],
    ...
    # ===== 追加類義語（Phase2拡充）=====
    # 電気設備 - ケーブル・配線
    "高圧ケーブル": ["CVケーブル", "CVT", "CV", "6600V", "6kV", "高圧CVT", "高圧CV", "架橋ポリエチレン"],
    "低圧ケーブル": ["CVケーブル", "VVF", "VVR", "EM-CE", "600V"],
    "ケーブル撤去": ["撤去工事", "既設撤去", "既存撤去", "ケーブル撤去工事"],
    "配線撤去": ["撤去工事", "既設撤去", "電線撤去", "配線撤去工事"],
    "ケーブル端末": ["端末処理", "端末処理材", "ケーブルヘッド", "終端処理"],
    "電線接続": ["ジョイント", "スリーブ", "圧着端子", "端子台接続"],
    "ケーブル布設": ["ケーブル敷設", "配線工事", "ケーブル引込"],

    # 電気設備 - 盤・機器
    "高圧盤": ["キュービクル", "受変電設備", "受電盤", "VCB盤"],
    "低圧盤": ["分電盤", "動力盤", "電灯盤", "配電盤", "低圧配電盤"],
    "制御盤工事": ["盤据付", "盤設置", "制御盤設置", "盤工事"],
    "盤撤去": ["分電盤撤去", "盤撤去工事", "既設盤撤去"],
    "盤内配線": ["盤内結線", "内部配線", "盤結線"],

    # 電気設備 - 照明・コンセント
    "照明器具取付": ["照明取付", "器具取付", "照明設置", "LED取付"],
    "照明撤去": ["器具撤去", "既設照明撤去", "照明撤去工事"],
    "スイッチ": ["照明スイッチ", "ホタルスイッチ", "片切スイッチ", "3路スイッチ"],
    "コンセント工事": ["コンセント設置", "コンセント取付", "電源コンセント"],
    "OAフロア": ["フリーアクセスフロア", "二重床", "OA床"],

    # 電気設備 - 弱電・防災
    "火災報知器": ["火報", "自火報", "感知器", "煙感知器", "熱感知器", "発信機"],
    "発信機": ["P型発信機", "火災発信機", "手動発信機"],
    "受信機": ["火災受信機", "P型受信機", "R型受信機", "表示盤"],
    "非常放送": ["非常用放送", "非常放送設備", "放送設備"],
    "スピーカー": ["放送スピーカー", "天井スピーカー", "壁掛スピーカー"],
    "電話配線": ["電話回路", "電話工事", "電話設備"],
    "テレビ配線": ["TV配線", "CATV配線", "テレビ共聴"],

    # 機械設備 - 空調
    "空調工事": ["空調設備工事", "エアコン工事", "冷暖房工事"],
    "空調機器": ["エアコン", "空調機", "室内機", "室外機"],
    "天井カセット": ["天カセ", "天井埋込", "4方向カセット", "天井カセット形"],
    "壁掛けエアコン": ["壁掛形", "壁掛型", "ルームエアコン"],
    "床置きエアコン": ["床置形", "床置型", "フロアコン"],
    "冷媒工事": ["冷媒配管工事", "冷媒チャージ", "冷媒充填"],
    "エアコン撤去": ["空調機撤去", "室内機撤去", "室外機撤去"],

    # 機械設備 - 換気
    "換気設備": ["換気工事", "換気扇工事", "給排気設備"],
    "給気": ["給気ファン", "給気口", "給気設備", "OA"],
    "排気": ["排気ファン", "排気口", "排気設備", "EA"],
    "送風機": ["ファン", "ブロワー", "シロッコファン", "ターボファン"],
    "厨房排気": ["厨房換気", "レンジフード", "グリスフィルター"],
    "ベンチレーター": ["換気扇", "軸流ファン", "有圧換気扇"],
    ...
    "給水引込": ["水道引込", "引込工事", "本管接続"],
    "排水接続": ["下水接続", "公共桝接続", "桝接続"],
    "配管撤去": ["既設配管撤去", "撤去工事", "配管撤去工事"],
    "配管更新": ["配管改修", "配管取替", "配管更生"],
    "バルブ": ["止水弁", "仕切弁", "ゲートバルブ", "ボール弁"],
    "メーター": ["水道メーター", "流量計", "量水器"],
    "洗面台": ["洗面器", "洗面化粧台", "手洗器"],
    "便器": ["大便器", "洋式便器", "和式便器", "ウォシュレット"],
    "小便器": ["ストール小便器", "壁掛小便器"],
    "流し": ["流し台", "シンク", "厨房シンク", "SK"],

    # ガス設備 - 追加
    "ガス配管": ["ガス管", "都市ガス配管", "LPガス配管"],
    "ガス工事": ["ガス設備工事", "ガス配管工事"],
    "ガス栓設置": ["ガス栓取付", "ガスコンセント設置"],
    "ガス管撤去": ["既設ガス管撤去", "ガス配管撤去"],
    "ガス試験": ["気密試験", "ガス漏れ試験", "圧力試験"],
    "フレキ管": ["フレキシブル管", "ガスフレキ", "金属フレキ"],
    "継手": ["ソケット", "エルボ", "チーズ", "レジューサ", "ニップル"],

    # 共通工事
    "撤去工事": ["解体工事", "撤去", "既設撤去", "既存撤去"],
    "新設工事": ["新設", "設置工事", "取付工事"],
    "更新工事": ["取替工事", "交換工事", "改修工事"],
    "試験調整": ["試運転", "調整", "検査", "動作確認"],
    "電気試験": ["絶縁試験", "接地抵抗試験", "耐圧試験"],
    "消防検査": ["消防届", "消防法検査", "消防設備点検"],
    "竣工検査": ["完了検査", "検査立会", "官庁検査"],
    "仮設電気": ["仮設電源", "工事用電源", "仮設配電"],
    "仮設水道": ["仮設給水", "工事用水", "仮設配管"],
    "足場工事": ["足場組立", "足場解体", "仮設足場"],
    "養生工事": ["養生", "床養生", "壁養生", "保護"],
    "清掃工事": ["清掃", "クリーニング", "美装工事"],
    "産廃処分": ["産業廃棄物処分", "廃棄物処理", "残材処分"],

    # 工事区分
    "電気工事": ["電気設備工事", "電気設備"],
    "機械工事": ["機械設備工事", "機械設備"],
    "衛生工事": ["衛生設備工事", "衛生設備", "給排水工事"],
    "空調工事": ["空調設備工事", "空調設備", "冷暖房工事"],
    "消防工事": ["消防設備工事", "消防設備", "防災工事"],
    "ガス工事": ["ガス設備工事", "ガス設備", "都市ガス工事"],

    # 単位関連
    "穴あけ": ["穴あけ工事", "開口", "コア抜き", "ホールソー"],
    "穴補修": ["穴埋め", "開口補修", "貫通部処理", "モルタル充填"],
    "壁貫通": ["貫通工事", "スリーブ", "貫通処理"],
    "床貫通": ["床開口", "床スリーブ", "床貫通処理"],
    "防火区画貫通": ["区画貫通", "耐火処理", "防火処理"],
}
...
MAX_PRICE_ITEMS = {
    ...
    "照明器具": 80000,            # 最大8万円/台（旧: 15万円）
    "LED照明": 80000,             # 最大8万円/台
    "感知器": 50000,              # 最大5万円/個
    "配線": 15000,                # 最大1.5万円/m（旧: 5万円）
    "ケーブル": 20000,            # 最大2万円/m（旧: 3万円）
    "接続": 300000,               # 最大30万円/式（旧: 50万円）
    "テレビ端子": 15000,          # 最大1.5万円/箇所
    "分電盤": 150000,             # 最大15万円/面
    "試運転": 200000,             # 最大20万円/式
    "試験調整": 200000,           # 最大20万円/式
    # 機械設備
    "配管": 10000,                # 最大1万円/m（一般配管）
    "エアコン": 200000,           # 最大20万円/台（業務用除く）
    "換気扇": 50000,              # 最大5万円/台
    "給湯器": 300000,             # 最大30万円/台
    # ガス設備
    "ガス管": 15000,              # 最大1.5万円/m
...
class AIEstimateGenerator:
    ...
    def _validate_price(self, item_name: str, matched_price: float) -> bool:
        ...
        # グローバル上限チェック: 100万円を超える単価は危険
        # （キュービクル等の高額機器は除外キーワードでスキップ済み）
        GLOBAL_MAX_PRICE = 1000000  # 100万円
        if matched_price > GLOBAL_MAX_PRICE:
            logger.warning(
                f"Price validation failed: '{item_name}' matched ¥{matched_price:,.0f} "
                f"exceeds global maximum ¥{GLOBAL_MAX_PRICE:,.0f}"
            )
            return False

        return True

    def _check_unit_compatibility(self, item_unit: str, kb_unit: str, kb_price: float = None, kb_item: dict = None) -> bool:
        """
        単位の互換性をチェック（強化版）
        ...
        Args:
            item_unit: 見積項目の単位
            kb_unit: KB項目の単位
            kb_price: KB項目の単価（高額「式」単価チェック用）
            kb_item: KB項目全体（高額一式フラグチェック用）
        ...
        # 500万円超の高額一式項目は完全一致が必要（フラグチェック）
        if kb_item and kb_item.get("requires_exact_match"):
            logger.debug(f"Rejecting high-value lump sum item requiring exact match: ¥{kb_price:,.0f}")
            return False

        # 「式」単位の高額単価チェック（重要！）
        # KB側が「式」で50万円以上の場合、建物全体の一式金額の可能性が高い
        # この場合、マッチングを拒否して誤った高額適用を防ぐ
        if kb_price and "式" in unit_norm_kb and kb_price >= 500000:
            logger.debug(f"Rejecting high-value '式' unit: ¥{kb_price:,.0f}")
            return False
...
【数量推定ルール】（学校施設の標準値）
※重要: 小規模施設（1000㎡未満）は数量を控えめに。過大な数量は見積精度を低下させます。

■ 電気容量の目安
  - 学校施設: 30〜50 VA/㎡（小規模仮設は30 VA/㎡）
  - キュービクル容量: 延床面積(㎡) × 30 VA/㎡ ÷ 0.7（需要率）→ kVA換算

■ 受変電設備
  - キュービクル: 1台（300kVA未満は1面）
  - 高圧ケーブル: 引込柱〜キュービクル間（通常20〜50m）
  - PAS（気中開閉器）: 1台
...
■ 照明設備（※控えめに推定）
  - LED照明器具: 床面積 ÷ 30㎡/台（最大50台まで）
  - 非常照明: 階数 × 3台
  - 誘導灯: 2台/階 × 階数

■ コンセント・配線（※控えめに推定）
  - コンセント: 床面積 ÷ 30㎡/箇所（最大50箇所まで）
  - 配線: 総延長100〜200m程度
        ...
        for item in estimate_items:
            ...
            if vector_search_available:
                ...
                if vector_result:
                    ...
                    kb_price = kb_item.get("unit_price")
                    # 単位互換性チェック（高額「式」単価を拒否）
                    if self._check_unit_compatibility(item.unit, kb_item.get("unit", ""), kb_price, kb_item):
                        # 単価妥当性チェック
                        if self._validate_price(item.name, kb_price):
        ...
        for item in estimate_items:
            ...
            if not matched_item:
                ...
                for kb_item in self.price_kb:
                    ...
                    # 4. 単位互換性チェック（高額「式」単価を拒否）
                    kb_price = kb_item.get("unit_price")
                    if not self._check_unit_compatibility(item.unit, kb_item.get("unit", ""), kb_price, kb_item):
                        continue
